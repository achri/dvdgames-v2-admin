...
class Lib_files {
	...
	function upload_ajax($folder='temp') {
		...
		$filebefore = $this->CI->input->get_post('gambar');
		...
		if(@is_uploaded_file($_FILES['userfile']['tmp_name'])) {
			...
			log_message('debug','upload_ajax->'.$filebefore.'|'.$filename);
			
			if (@move_uploaded_file($_FILES['userfile']['tmp_name'], $target_folder.$filename)) {
				$this->delete_all_images('temp',$filename);
				return $filename;
			} 
		}
	}
	...
	function delete_all_images($folder='temp',$curFile=FALSE) {
		$target_folder	= 'storage/'.$folder.'/';
